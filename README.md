...
**Current state: Milestone 1 (provenance + data ingestion).** No scientific
inference is implemented yet.
...
python -m pytest tests -q                    # 55 tests, no network needed
```
...
src/ocen_dm/   package: paths, provenance, data loaders, CLI
...
Not yet installed and needed later: `jampy` (Milestone 3), `ultranest`/`dynesty`
(Milestone 4), `pooch`, `snakemake`.
