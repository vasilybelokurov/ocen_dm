...
$\beta_0\le-1/2$ is imposed. \citet{anevans2006} give two conditions. For a self-gravitating
cusp, $\beta\le\gamma/2$, so a cored tracer needs $\beta_0\le0$. But in a potential dominated
by a central point mass the condition is $\gamma\ge\beta+1/2$, so a cored tracer needs
$\beta_0\le-1/2$. Every model here carries a point mass, and its sphere of influence is
\emph{not} safely unresolved: it reaches the innermost datum at $0.11$\,pc once
$M_\bullet>7400\,\Msun$, which is 46 per cent of the prior. The tighter ceiling is applied to
both families equally. It is a necessary, not sufficient, condition for a non-negative DF.
...
Plummer radius, so its central limit is not Keplerian. It is run as a diagnostic on posterior
draws, not as a veto --- vetoing would define a different model and require its own evidence.
...
They are taken one rung at a time, following \texttt{docs/MODELLING\_PLAN.md}: constant
anisotropy without instrument scales first (6 and 8 parameters against 89 points), then the
three-parameter $\beta(r)$, then the scales. The decision to climb is made on the residuals
of the rung below, not in advance. An anisotropy turnover and a shared rotation nuisance are
written down there as deferred, with the conditions that would bring them back.
