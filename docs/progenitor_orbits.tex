...
the system like a test particle only for bound masses $\lesssim10^7\,\Msun$: a
$10^8\,\Msun$ remnant is never captured by the resonance, so a class-3 nucleus arrived from
GSE essentially without a dark envelope. The classes differ most in the pericentric tides
...
$M=10^7\,\Msun$ & 0.92 & 0.99 & 0.05 & 0.59 & 0.99 & 1.00 & 0.89 & 0.28 & 0.00\\
$M=10^8\,\Msun$ & 0.00 & 0.20 & 0.14 & 0.33 & 0.23 & 0.00 & 0.00 & 0.00 & 0.00\\
$M=10^9\,\Msun$ & \multicolumn{9}{c}{0.00 at $\Ob=24$; orbit pumped to a 74\,kpc apocentre}\\
\bottomrule\end{tabular}
\end{table}
...
The answer is mass-dependent with a sharp threshold between $10^7$ and $10^8\,\Msun$:
\begin{itemize}
\item $M\le10^7\,\Msun$: migrates like a test particle. The $\Ob$ window is the same
  ($\lesssim26$), but the pre-migration endpoint moves by $\Delta E\approx+0.13\times10^5$ and
  $\Delta\Lz\approx+240$\,kpc\,\kms\ over 8\,Gyr (roughly the friction drift), to the
  outer edge of the most-bound GSE debris.
\item $M=10^8\,\Msun$: no pattern speed brings more than a third of the samples into GSE, and
  none does at the fiducial $\Ob=24$--27. The resonant torque and the friction torque are
  comparable and the resonance loses.
...
\end{itemize}
Since the constant mass overstates the friction of a stripping system, the threshold for a
real remnant is somewhat higher than $10^8\,\Msun$, but the conclusion stands: \emph{the
class-3 initial condition is self-consistent only if the nucleus had already lost all but
$\lesssim10^{7\text{--}8}\,\Msun$ of its bound envelope before the bar slowed}, i.e.\ before
...
axisymmetric & 1.7 / 7.0--7.3 & 1.9--3.4 / 7--18 & 73--94\\
bar, $\Ob=37.5$ & 1.9 / 7.0--7.3 & 1.8--3.1 / 8--19 & 80--99\\
bar, $\Ob=24$ & 0.85--1.07 / 10--12 & 0.9--2.1 / 12--26 & 95--112\\
\bottomrule\end{tabular}
\end{table}

With the fast bar nothing changes beyond the sample scatter: the class-2 picks stand. With
the slow bar the last $\sim$2.5\,Gyr become class-3-like (pericentre 1\,kpc, apocentre
10--12), because the retrograde 1:1 resonance sits at exactly this pattern speed for today's
