...
(pericentre 0.8 rather than 1.6\,kpc). Adding dynamical friction shows that the bar migrates
the system like a test particle for bound masses $\lesssim10^7\,\Msun$, only marginally at
$10^8$ and not at all at $10^9$, so a class-3 nucleus arrived from GSE with at most
$\sim10^8\,\Msun$ of dark envelope. The classes differ most in the pericentric tides
...
the class-3 integration with Chandrasekhar friction for a constant bound mass $M$ (an upper
bound on the friction of a system still losing mass), evaluated with the time-dependent,
...
carries the system, at a visibly shifted endpoint; at $10^8\,\Msun$ it never does --- friction
detunes the orbit while the resonance sweeps past, and $\Lz$ drifts monotonically more
retrograde; at $10^9\,\Msun$ friction dominates everything.}
\label{fig:friction}
\end{figure}

The answer is mass-dependent, with the transition between $10^7$ and $10^8\,\Msun$:
\begin{itemize}
\item $M\le10^7\,\Msun$: migrates like a test particle. The $\Ob$ window is the same
  (0.8--1.0 for 20--25, 0.64 at 26, 0 at 27), and the pre-migration endpoint moves by
  $\Delta E\approx+0.12\times10^5$\,km$^2$\,s$^{-2}$ (the friction drift) with $\Lz$
  unchanged, staying inside the most-bound GSE debris.
...
\end{itemize}
Since the constant mass overstates the friction of a stripping system, a real remnant of
$10^8\,\Msun$ is somewhat less affected than this, but the conclusion stands: \emph{the
class-3 initial condition is self-consistent only if the nucleus had already lost all but
$\lesssim10^{8}\,\Msun$ of its bound envelope before the bar slowed}, i.e.\ before
$\sim$2.5\,Gyr ago. A nucleus that migrated from GSE debris did so essentially naked. This is
a strong statement about the dark-matter content reachable by class~3, and it follows from the
same physics that makes friction negligible for the nucleus alone.
...
\end{equation}
and the remnant density profile converges to an exponentially truncated cusp,
...
$M_{\rm mx}/M_{\rm mx,0}\sim10^{-12}$ within a few passages --- numerically meaningless, but
the direction is robust and follows from the density criterion alone. A self-bound remnant
must reach $\simeq20\,\bar\rho_{\rm host}(<r_{\rm peri})$, which is $1.7\times10^{10}$
...
($2.4\times10^{3}$). \emph{On every one of the nine orbits a bare dark-matter halo is stripped
to a self-bound remnant far below the nucleus mass.} Cored haloes fare worse
\citep{errani2020}. Whatever dark matter surrounds \ocen\ today is therefore not the remnant
of a subhalo holding itself together; it is material held by the nucleus's own potential.
...
is what the nucleus can retain if nothing else removes it (Table~\ref{tab:budget},
Fig.~\ref{fig:budget}).
...
class and halo --- the upper envelope of what the nucleus can hold.}
\label{fig:budget}
\end{figure}

Three consequences. (i) The class ranking is quantitative: a class-3 history caps the dark
matter at $\lesssim4\times10^{5}\,\Msun$ ($10^{10}$, $c=5.5$; $10^{6}$ for $c=12$), classes 1--2
at $\lesssim1.5$ ($4.4$)$\times10^{6}$ --- of order the nucleus mass, and spread over 70\,pc
rather than 7. (ii) These are upper envelopes without adiabatic contraction of the cusp by
the nucleus's formation (which raises them) and without the pericentric shocks acting on the
material inside $r_J$ (which lower them); the balance between the two is precisely what the
$N$-body runs must settle. (iii) The $N$-body problem shrinks: only the inner $\sim100$\,pc
needs to be live --- $\sim2\times10^{6}\,\Msun$ of dark matter in a $10^{10}$ halo, which
$10^{5}$--$10^{6}$ particles of $\sim10\,\Msun$ resolve --- while the outer halo, whose fate the
tracks already give, can be an analytic potential that is switched off as it is stripped. The
multi-mass difficulties of Sec.~\ref{sec:friction} largely disappear.
...
\item Corrections after the Codex review of 2026-09-20: the leapfrog unit bug above; the class-3
  Jacobi radius at the 0.4\,kpc pericentre is 33\,pc (59 with the $d\ln M/d\ln r$ term), not
  the 50--90\,pc that holds at 1.6\,kpc; two of the class-2 stripping histories end today at
