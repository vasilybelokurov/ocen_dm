...
controlled by the satellite's mass-loss history rather than its mass, and the selected histories
converge to today's orbit over the last 3--5\,Gyr; class 3 reproduces the published result
...
(pericentre 0.8 rather than 1.6\,kpc). Constant-mass friction experiments show a substantial change between
$10^7$ and $10^8\,\Msun$ and no GSE overlap for the tested $10^9\,\Msun$ case.
This motivates early envelope loss but does not establish a universal mass bound. The classes differ most in the pericentric tides
...
\texttt{writeup\_figures.py} (tables and figures). Tests: \path{tests/test_progenitor_orbits.py},
\path{tests/test_bar_migration.py} (the original validation was recorded in the journal). Numerical products are in
\texttt{results/tails/}.

\section{Present-day phase-space point}
\label{sec:present}

The adopted centre from \texttt{cluster.py} is
$\alpha=201.69683^\circ$, $\delta=-47.47957^\circ$.
All classes use distance $D=5.43\pm0.05$\,kpc, systemic
...
X&=\frac{v}{\sqrt2\,\sigma},\qquad \sigma=\frac{v_{\rm c}(r)}{\sqrt2},\qquad
\ln\Lambda=\ln\frac{r}{\max(r_{\rm h},\,GM/v^2)} .
\end{align}
...
class~1 to four figures (1.573/7.037 versus \texttt{agama.orbit} 1.572/7.037\,kpc in the same
potential), and a constant-mass case checked against \texttt{galpy}'s
\texttt{ChandrasekharDynamicalFrictionForce} agrees to 0.1\% in pericentre and 4\% in apocentre
(1.786/44.3 versus 1.786/46.0\,kpc; 1\,s versus 482\,s). (A first version of the integrator
converted accelerations with the wrong power of the time unit, making kicks 4.4\% too weak;
Codex caught it on 2026-09-20 and every leapfrog result in this note was regenerated.)
...
Sequoia-like & $10^{10}$ & 2 & 65 & 3.0--18.1 & 1.9--9.2\\
intermediate & $3\times10^{10}$ & 1.2 & 86 & 2.2--11.7 & 1.6--7.3\\
GSE-like & $10^{11}$ & 0.75 & 77 & 1.7--7.4 & 1.6--7.0\\
\bottomrule\end{tabular}
\end{adjustbox}
\end{table}
...
\item \textbf{Time unit.} All class-3 times follow the paper's convention and are in
  kpc/(\kms) $=0.978$\,Gyr, so ``8\,Gyr'' means $8\times0.978=7.8$\,Gyr and $\eta$ is per
  that unit; the labels in this section keep the paper's rounding.
...
\item Cross-check against the authors' stored grid (\path{oCen_bar/artifacts/03_orbit_grid.npz}):
...
\subsection{Does a massive remnant migrate like a test particle?}
\label{sec:friction}
The back-integration treats \ocen\ as a test particle. The bound system whose dark-matter
content we want to know is nucleus $+$ remnant halo, and the one process that distinguishes it
from a particle at leading order is dynamical friction, which scales with its mass. We repeated
the class-3 integration with Chandrasekhar friction for a constant bound mass $M$ (a controlled sensitivity experiment, not a bound on an unspecified mass-loss history), evaluated with the time-dependent,
bar-inclusive density, $\sigma=v_{\rm c}/\sqrt2$ of the axisymmetric potential and
$r_{\rm h}=0.1\,{\rm kpc}\,(M/10^8\,\Msun)^{1/3}$; 100 samples, vectorised leapfrog with
0.25\,Myr steps in the AGAMA potential (\texttt{tails/friction\_test.py}). At the nucleus
mass the implementation gives $\dot\Lz=11$\,kpc\,\kms\,Gyr$^{-1}$ and
$\dot E=7\times10^2$\,km$^2$\,s$^{-2}$\,Gyr$^{-1}$, matching the paper's 9.6 and
$6.3\times10^2$; the frictionless leapfrog reproduces \texttt{agama.orbit} (0.87 versus 0.89
inside GSE).

\begin{table}[h]\centering
\caption{Fraction of samples inside the outer GSE contour 8\,Gyr ago with friction for a
constant bound mass. Test-particle values from Table~\ref{tab:c3grid} for comparison.}
\label{tab:friction}
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{l*{9}{c}}\toprule
$\Ob$ [\kmskpc] & 20 & 21 & 22 & 22.5 & 23 & 24 & 25 & 26 & 27\\\midrule
test particle & 0.44 & 0.38 & 0.58 & 0.85 & 0.01 & 0.89 & 0.79 & 0.18 & 0.00\\
$M=10^7\,\Msun$ & 0.81 & 0.55 & 0.77 & 0.02 & 0.89 & 0.93 & 0.98 & 0.64 & 0.03\\
$M=10^8\,\Msun$ & 0.00 & 0.07 & 0.47 & 0.15 & 0.44 & 0.34 & 0.00 & 0.00 & 0.00\\
$M=10^9\,\Msun$ & \multicolumn{9}{c}{0.00 at $\Ob=24$; orbit pumped to a 79\,kpc apocentre}\\
\bottomrule\end{tabular}
\end{adjustbox}
\end{table}

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{friction_test_elz.png}
\caption{Massive-remnant test at $\Ob=24$. Top: $(\Lz,E)$ of the samples 8\,Gyr ago for
$M=0$, $10^7$, $10^8$, $10^9\,\Msun$ (green inside, red outside the outer GSE contour).
Bottom: $\Lz(t)$ of every tenth sample. At $10^7\,\Msun$ the resonance still captures and
carries many samples, with shifted endpoints. At $10^8\,\Msun$ the fraction inside the
GSE contour is 0.34, compared with 0.93 at $10^7\,\Msun$; at $10^9\,\Msun$ it is zero.
Contour membership is an endpoint diagnostic, not by itself proof of resonance capture.}
\label{fig:friction}
\end{figure}

The answer is mass-dependent, with the transition between $10^7$ and $10^8\,\Msun$:
\begin{itemize}
\item $M=10^7\,\Msun$: high GSE-overlap fractions at several pattern speeds, but a
  strong dip at $\Ob=22.5$. Friction changes the resonance-sensitive window rather than
  preserving the test-particle grid point by point.
\item $M=10^8\,\Msun$: marginal. At $\Ob=22$--24 a third to a half of the samples still end
  inside GSE, none for $\Ob\ge25$; the resonant and friction torques are comparable and the
  endpoint is pushed to $E\approx-0.8\times10^5$, the upper edge of the debris.
\item $M\ge10^9\,\Msun$: friction dominates; the system would have come from the outer halo
  (class~2 territory), not from GSE debris.
\end{itemize}
These constant-mass experiments place the transition in sensitivity between
$10^7$ and $10^8\,\Msun$ for the chosen bar histories; they suggest substantial envelope
loss before migration. They do not define a universal $10^8\,\Msun$ upper limit. A real
mass history can exert more or less accumulated drag than a fixed-mass trial, and the
resonant response is not monotonic. Forward integrations with an explicit evolving mass
and size are needed to turn this diagnostic into a constraint.
...
\subsection{Which pattern speeds put today's orbit in resonance?}
\label{sec:resonances}
A different question from ``which $\Ob$ carries \ocen\ from GSE to its present $(E,\Lz)$'' is
``for which $\Ob$ is the \emph{present} orbit resonant''. From AGAMA's action finder in the
axisymmetrised Hunter et al.\ (2024) potential (1000 error samples;
\texttt{tails/resonances.py}) the present orbit has
$\Omega_r=69.7$, $\Omega_z=63.1$, $\Omega_\phi=-44.3$\,\kmskpc\ (retrograde; McMillan 2017
gives 70.2, 62.8, $-44.0$), with actions $J_r=242$, $J_z=120$, $\Lz=-533$\,kpc\,\kms. A
resonance with a prograde bar requires
\begin{equation}
l\,\Omega_r + n\,\Omega_z + m\,(\Omega_\phi-\Ob)=0
\quad\Longrightarrow\quad
\Ob=\Omega_\phi+\frac{l\,\Omega_r+n\,\Omega_z}{m},
\end{equation}
and because $\Omega_\phi<0$ only combinations with enough radial or vertical frequency give a
positive $\Ob$ (Table~\ref{tab:res}). The sample spread is $\pm0.1$\,\kmskpc; the potential
difference $\pm0.5$.

\begin{table}[h]\centering
\caption{Bar pattern speeds for which \ocen's present orbit is resonant.}
\label{tab:res}
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{lcl}\toprule
resonance $(l,n,m)$ & $\Ob$ [\kmskpc] & comment\\\midrule
corotation $(0,0,1)$ & $-44.3$ & impossible for a retrograde orbit\\
OLR-type $(1,0,2)$ & $-9.5$ & impossible\\
$(3,0,4)$ & 8.0 & far too slow\\
vertical $(0,1,1)$ & 18.7 & \\
$(1,1,2)$ & 22.1 & \\
\textbf{retrograde 1:1} $(1,0,1)$ & \textbf{25.4} & the Dillamore et al.\ (2026) resonance\\
$(5,0,4)$ & 42.8 & high order, weak\\
$(4,0,3)$ & 48.6 & \\
$(3,0,2)$ & 60.3 & \\
$(1,1,1)$ & 88.5 & \\
\bottomrule\end{tabular}
...
With a bar at the mainstream 33--41\,\kmskpc\ the present orbit is in no low-order
resonance (the nearest is the weak 5:4 at 43). The only strong resonance it can occupy is the
retrograde 1:1 at $\Ob=25.4$, which is why the migration scenario needs that pattern speed and,
equally, why a fast bar merely perturbs today's orbit (Sec.~\ref{sec:c2bar}).

\subsection{Class 2 in the barred potential}
\label{sec:c2bar}
The three class-2 histories were re-integrated in Hunter et al.\ (2024) --- axisymmetrised,
and with the growing, decelerating bar ending at the mainstream 37.5\,\kmskpc\ (Hunter's
value) and at 24 --- with 20 error samples each (\texttt{tails/class2\_barred.py},
Fig.~\ref{fig:c2bar}). The bar is 8\,Gyr old; before that the potential is axisymmetric.

\begin{table}[h]\centering
\caption{Class 2 in the barred potential (kpc; medians over samples and over the three
histories' ranges).}
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{lccc}\toprule
& last Gyr peri / apo & 5--6\,Gyr ago peri / apo & apocentre before infall\\\midrule
axisymmetric & 1.6--1.8 / 7.0--7.3 & 1.8--3.2 / 7--19 & 64--117\\
bar, $\Ob=37.5$ & 1.9--2.3 / 7.1--7.2 & 1.8--3.4 / 8--19 & 75--112\\
bar, $\Ob=24$ & 1.3--1.7 / 9.1--11.6 & 1.0--1.9 / 11--28 & 91--118\\
\bottomrule\end{tabular}
\end{adjustbox}
\end{table}

With the fast bar nothing changes beyond the sample scatter: the class-2 picks stand. With
the slow bar the last $\sim$2.5\,Gyr become class-3-like (pericentre 1.3--1.7\,kpc,
apocentre 9--12), because the retrograde 1:1 resonance sits at exactly this pattern speed for today's
orbit; the infall apocentre rises by $\sim$20\% but stays inside the 50--150\,kpc selection
band, so the same $(M_{\rm inf},\tau)$ picks remain valid. Under the slow bar the pericentres
differ from the axisymmetric case all the way back to infall; that is not the bar acting
9\,Gyr ago but a property of back-integration --- the whole inferred past is conditioned on
today's state, so a resonant kick in the last 2.5\,Gyr re-routes the entire earlier
trajectory.

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{class2_barred.png}
\caption{Class 2 in the Hunter et al.\ (2024) potential: axisymmetric (solid), bar ending at
37.5 (dashed) and at 24\,\kmskpc\ (dotted). Top: one sample's radius. Bottom: per-Gyr
pericentre (blue) and apocentre (red), median over 20 samples; the bar forms 8\,Gyr ago.}
\label{fig:c2bar}
\end{figure}
...
2 & Sequoia-like & 1.60 / 2.95 / 5.18 & 7.4 / 18.1 / 48.2 & 14.7 & 1.8 & 0.46\\
2 & intermediate & 1.59 / 2.24 / 5.36 & 7.1 / 11.7 / 43.1 & 14.8 & 1.7 & 0.46\\
2 & GSE-like & 1.58 / 1.69 / 4.54 & 7.0 / 7.4 / 39.5 & 14.9 & 2.3 & 0.46\\
3 & sample 986 & 0.84 / 0.59 / 0.40 & 9.1 / 11.0 / 10.9 & 39.4 & 105.2 & 0.33\\
3 & sample 133 & 0.80 / 0.71 / 0.42 & 9.3 / 11.7 / 11.6 & 42.0 & 98.6 & 0.32\\
3 & sample 240 & 0.84 / 0.80 / 0.45 & 9.1 / 11.1 / 12.2 & 39.4 & 90.0 & 0.33\\
\bottomrule\end{tabular}
\end{adjustbox}
\end{table}
...
\section{Tidal tracks: what the orbits imply before any $N$-body run}
\label{sec:tracks}

\subsection{The method and why it separates structure from orbit}
\citet{penarrubia2010} and \citet[EN21]{errani2021} showed that the structure of a
tidally stripped NFW subhalo depends only on how much mass it has lost, not on how: the
characteristic velocity and radius follow one track,
\begin{equation}
\frac{V_{\rm mx}}{V_{\rm mx,0}}=2^{\alpha}\Big(\frac{r_{\rm mx}}{r_{\rm mx,0}}\Big)^{\beta}
\Big[1+\Big(\frac{r_{\rm mx}}{r_{\rm mx,0}}\Big)^{2}\Big]^{-\alpha},\qquad \alpha=0.4,\ \beta=0.65,
\end{equation}
The associated remnant density is an exponentially truncated cusp,
$\rho=\rho_{\rm NFW}\,e^{-r/r_{\rm cut}}/(1+r_s/r_{\rm cut})^{0.3}$ with
$r_{\rm cut}/r_{\rm mx,0}=0.44\,(M_{\rm mx}/M_{\rm mx,0})^{0.44}[1-(M_{\rm mx}/M_{\rm mx,0})^{0.3}]^{-1.1}$.
The orbit enters only through the speed along the track: with the crossing time
$T_{\rm mx}=2\pi r_{\rm mx}/V_{\rm mx}$ and the circular time at pericentre
$T_{\rm peri}=2\pi r_{\rm peri}/V_{\rm c}$, EN21 find that the remnant converges to
$T_{\rm asy}\simeq0.22\,T_{\rm peri}$, i.e.\ to a mean density $\simeq20$ times the host's
mean density inside the pericentre, along $Y(t)=(T_{\rm mx}-T_{\rm asy})/T_{\rm peri}=
Y_0[1+(t/\tau)^\eta]^{-1/\eta}$ with $\tau=0.65\,T_{\rm orb}/Y_0$, $\eta=1-e^{-2.5Y_0}$, and
eccentric orbits delayed by $f_{\rm ecc}=[2x/(x+1)]^{3.2}$, $x=r_{\rm apo}/r_{\rm peri}$.
Because the state is a single number on the track, a time-varying orbit is handled passage by
passage with the local $(T_{\rm peri},T_{\rm orb},f_{\rm ecc})$ (\texttt{tails/tidal\_tracks.py},
\texttt{walk}); the host mean density inside each pericentre replaces the isothermal
$T_{\rm peri}$ (EN21's own suggested generalisation, with their caveat).

\subsection{Our orbits are outside the calibrated regime}
EN21 simulated $0.2<T_{\rm mx,0}/T_{\rm peri}<2$. For NFW haloes of $10^{9}$--$10^{11}\,\Msun$
at $z=2$ ($c=4.5$--$7$; $r_{\rm mx,0}=3$--$22$\,kpc, $T_{\rm mx,0}=0.8$--$1.4$\,Gyr) on our
orbits the ratio is $T_{\rm mx,0}/T_{\rm peri}=8$--$65$: the pericentric tides are so much
stronger than anything in the calibration that the walk extrapolates to remnants of
$M_{\rm mx}/M_{\rm mx,0}\sim10^{-12}$ within a few passages --- not calibrated predictions. The asymptotic density criterion provides a diagnostic
scale rather than a rigorous survival threshold in this regime: it gives $\simeq20\,\bar\rho_{\rm host}(<r_{\rm peri})$, which is $1.7\times10^{10}$
(classes 1--2, $r_{\rm peri}=1.6$\,kpc) to $1.5\times10^{11}\,\Msun\,{\rm kpc}^{-3}$ (class 3,
0.4\,kpc). An NFW cusp of $10^{10}\,\Msun$, $c=5.5$ has that mean density only inside 4.6\,pc
($6.7\times10^{3}\,\Msun$) for classes 1--2 and inside 0.5\,pc ($\sim10^{2}\,\Msun$) for
class 3; a concentrated $c=12$ halo inside 14\,pc ($1.7\times10^{5}$) and 1.6\,pc
($2.4\times10^{3}$). These small diagnostic scales motivate including the nucleus's gravity explicitly.
They do not establish the mass left by a full disruption simulation. Cored haloes are
more vulnerable in the calibrated models \citep{errani2020}; extrapolation to these
histories still needs validation.

\subsection{The dark-matter budget the nucleus can hold}
That reframes the question as a budget: the dark matter initially inside the nucleus's Jacobi
radius at the smallest pericentre, $r_J=r_{\rm peri}[M_{\rm nuc}/3M_{\rm host}(<r_{\rm peri})]^{1/3}$,
is an initial spatial budget, not a computed retained mass (Table~\ref{tab:budget},
Fig.~\ref{fig:budget}).

\begin{table}[h]\centering
\caption{Initial NFW dark matter inside the nucleus's Jacobi radius at the minimum pericentre
($\Msun$); range over the orbits of each class. Nucleus mass $3.55\times10^{6}$.}
\label{tab:budget}
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{lcccc}\toprule
& $r_J$ [pc] & $10^{9}$, $c=7$ & $10^{10}$, $c=5.5$ & $10^{11}$, $c=4.5$ \quad / \quad $10^{10}$, $c=12$\\\midrule
class 1 & 40--70 & $3$--$9\times10^{5}$ & $0.5$--$1.5\times10^{6}$ & $0.8$--$2.6\times10^{6}$ / $1.5$--$4.4\times10^{6}$\\
class 2 & 70 & $9\times10^{5}$ & $1.5\times10^{6}$ & $2.6\times10^{6}$ / $4.4\times10^{6}$\\
class 3 & 33--36 & $2.2$--$2.5\times10^{5}$ & $3.5$--$4.0\times10^{5}$ & $5.9$--$6.7\times10^{5}$ / $1.0$--$1.2\times10^{6}$\\
\bottomrule\end{tabular}
\end{adjustbox}
\end{table}

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{tidal_tracks_budget.png}
\caption{Left: initial NFW mass profiles at $z=2$ with the self-bound-remnant scale from the
EN21 density criterion (grey) and the nucleus's Jacobi radius at the class-3 (pink) and
class-1/2 (blue) pericentres. Right: the dark matter initially inside that Jacobi radius, by
class and halo. These are initial aperture masses under the stated tidal-radius proxy.}
\label{fig:budget}
\end{figure}

The initial aperture budgets differ between classes and can be comparable to the nuclear
mass. They omit energy and angular-momentum selection, self-consistent re-adjustment,
contraction, shocks and stellar heating. They are not rigorous upper limits for arbitrary
formation histories, and DM self-gravity is not negligible for every grid point.

A smaller experiment can focus on the inner $\sim100$\,pc using test particles or a live
DM component around an extended nucleus. This is a proposed reduction, not a consequence
already validated by the tidal tracks. Particles starting farther out can pass through
the centre, and an imposed loss of the outer potential needs explicit justification.
The current validation sequence is in \texttt{docs/SECTION11\_TESTS.md}; the companion
WD note distinguishes an illustrative spatial taper from a calculated truncated DF.

\section{Caveats and reproducibility}
\begin{itemize}
\item Corrections after the Codex review of 2026-09-20: the leapfrog unit bug above; the class-3
  tidal-radius proxy at the 0.4\,kpc pericentre is about 33\,pc. A historical alternative
  estimate of 59\,pc has not been reproduced by a consistent tidal-tensor calculation
  and is not adopted. For circular spherical orbits the denominator coefficient is
  $3-d\ln M/d\ln R$; eccentric barred orbits need an explicitly stated approximation; two of the class-2 stripping histories end today at
  $6.7\times10^7$ and $1.0\times10^7\,\Msun$ rather than at the nucleus mass (a constraint
  worth imposing in a revision).
...
\item The \texttt{galpy} checkout used (2024-03) needs two import shims (astroquery metadata,
  scipy \texttt{vectorize1}); both live in \path{progenitor_orbits._import_galpy_safely}.
\item Products: \path{python -m ocen_dm.tails.products}, \path{python -m ocen_dm.tails.bar_migration},
  \path{python -m ocen_dm.tails.writeup_figures}. The class-3 run of $14\times10^3$ orbits
...
\bibitem[Errani \& Pe\~narrubia(2020)]{errani2020} Errani, R., Pe\~narrubia, J. 2020, MNRAS, 491, 4591, \url{https://arxiv.org/abs/1906.01642}
\bibitem[Errani \& Navarro(2021)]{errani2021} Errani, R., Navarro, J. F. 2021, MNRAS, 505, 18, \url{https://arxiv.org/abs/2011.07077}
\bibitem[Pe\~narrubia et al.(2010)]{penarrubia2010} Pe\~narrubia, J., Benson, A. J., Walker, M. G., et al. 2010, MNRAS, 406, 1290, \url{https://arxiv.org/abs/1002.3376}
