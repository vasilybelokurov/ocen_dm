...
4\,$\Msun\pc^{-3}$), driven by the kinematics. Twelve realistic mocks show, however, that
$\rho_{20}$ is recovered with a scatter of 0.5--0.9\,$\Msun\pc^{-3}$ when the
halo scale is free, as large as the observed rise, while the total mass
profile is recovered to a few per cent over 2--40\pc. The data therefore
determine the mass profile but not yet the stellar/DM split; we identify the
external constraints (a shape separation of DM from remnants, a stellar-mass
prior from the mass function) needed to resolve it.
\end{abstract}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{df_family_gallery_20260923.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{observed_df_wide_20260923_best_fits.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=.72\linewidth]{df_subset_beta_20260923.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=.72\linewidth]{action_coverage_observed_df_wide_20260923.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=.72\linewidth]{dftwo_beta_20260923.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{df_structure_dftwo_20260923_observed_no_halo_start0.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{best_fit_performance_20260923_data.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{best_fit_performance_20260923_diagnostics.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{counts_fit_performance_20260923_data.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{central_bh_evidence_20260923.png}
...
\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{rho20_profile_20260923.png}
...
\subsection{Mock validation and coverage}
\label{sec:mocks}

Mocks were built from the counts-based best fit, each with the published
kinematic noise (Gaussian per bin) and Poisson counts drawn with the fitted
amplitudes. Truth A is the fitted model itself ($\rho_{20}=0$); truth B adds
a halo with $\rho_{20}=1$, $r_s=30\pc$. Six noise realizations of each were
fitted with a free halo from generic hand-picked starts, never from the
truth (Figure~\ref{fig:coverage}; \path{bin/run_mock_coverage.sh}).

\begin{figure}[htbp]
\centering\includegraphics[width=\linewidth]{mock_coverage_20260923.png}
\caption{Recovery in twelve realistic mocks. (a) Recovered against true
$\rho_{20}$ (colour: profiled $r_s$); the red lines mark where the observed
profile crosses $\Delta=1$ and 3.84. (b) Recovered/true total enclosed mass
against radius (grey: beyond the data). (c) Recovered against true DM mass
inside 20 and 63\pc. (d) Fractional errors of the stellar and remnant
masses.}
\label{fig:coverage}
\end{figure}

\begin{itemize}
\item \textbf{$\rho_{20}$ is weakly identified.} For truth A the recovered
  values are 0.00, 0.01, 0.12, 0.21, 0.67 and 1.33 (mean 0.39, scatter
  0.52); for truth B they are 0.00, 0.16, 0.46, 0.79, 2.06 and 2.15 (mean
  0.94, scatter 0.94). The estimator is roughly unbiased in the mean but its
  scatter is as large as the signal: one no-halo realization returned
  $\rho_{20}=1.33$ (a compact 7\pc halo taking 6\% of the stellar mass and
  adding 21\% to the remnants), and one $\rho_{20}=1$ realization returned
  exactly zero.
\item \textbf{The total mass profile is what the data measure.} In all
  twelve mocks the total enclosed mass is recovered within $\pm3$\% at 20\pc
  and within $\pm5$\% over 2--40\pc, whatever the fitted split between
  stars, remnants and halo. Inside 1\pc and beyond 60\pc the recovery
  degrades (to $\pm25$\% and, in one case, $+42$\% at 63\pc when a
  $\rho_{20}=2$, $r_s=420\pc$ halo was fitted).
\item \textbf{The trade runs through the dark components and $M_\star$.}
  Stellar masses are recovered within $\pm6$\%, remnant masses within
  $\pm23$\%, and the errors are anti-correlated with the fitted halo.
\end{itemize}

Consequence for the observed profile (Section~\ref{sec:profile}): its rise
of $\Delta=2.2$ at $\rho_{20}=1$ lies within the estimator's noise scatter and
cannot be quoted as a limit on $\rho_{20}$. What these data determine is the
total mass profile over 2--40\pc; the stellar/DM split at 20\pc is not
resolved with a free halo scale. Breaking the degeneracy needs information
the kinematics do not carry: a shape separation between DM (extended, not
mass-segregated) and remnants (compact), and an external prior on $M_\star$
from the stellar mass function (Section~\ref{sec:conclusions}).

\subsection{What can and cannot be said}
\begin{itemize}
\item \textbf{Can:} under this model and the published errors, the total
  enclosed mass is determined to a few per cent over 2--40\pc, and the best
  fit has no halo.
\item \textbf{Cannot:} quote a limit on $\rho_{20}$. The mocks show a
  recovery scatter of 0.5--0.9\,$\Msun\pc^{-3}$ with a free halo scale, as
  large as the observed profile's rise; a compact ``halo'' is degenerate
  with the remnant sphere and with $M_\star$. The likelihood is also not
  calibrated, rotation is subtracted rather than modelled, and the
  tracer/mass assumption is untested.
\end{itemize}
...
\item With the count likelihood and the distance prior, a free halo goes to
  ...
  most compact halo allowed. Twelve realistic mocks recover $\rho_{20}$ with
  a scatter of 0.5--0.9 (no-halo truths returned up to 1.33; a $\rho_{20}=1$
  truth returned 0), while the total mass profile is recovered within
  $\pm5$\% over 2--40\pc. The observed profile is therefore not a
  $\rho_{20}$ limit; the data determine the mass profile, not the split.
\end{enumerate}
...
(i) A physical shape separation: DM cannot be mass-segregated, so
$r_s$ should not fall below the stellar half-mass radius ($\sim10\pc$),
whereas remnants are compact; this removes the compact-halo/remnant
degeneracy. (ii) An external prior on $M_\star$ from the present-day mass
function measured in the oMEGACat photometry (complete to $F625W\simeq23$,
i.e.\ $\sim0.45\,\Msun$), with white dwarfs included in the luminous
component and NS/BH remnants bounded by retention. (iii) Repeat the mocks
with both to measure the remaining scatter. (iv) Intervals only after the
likelihood is calibrated (objective 1, item ii).
...
\texttt{cov\_A\_s\{11..15\}\_20260923}, \texttt{cov\_B\_rho1.0\_s\{11..15\}\_20260923} & \texttt{f4fd4c7}+ & Coverage mocks (one hand start each)\\
\bottomrule
\end{tabularx}
\end{table}
