...
anisotropy. This is a mass--anisotropy trade, not a DM measurement. We document
the data issues that limit a statistical verdict: very small formal errors,
approximate rotation corrections, a heterogeneous photometric splice and
correlated bins. We also document the numerical defects found and fixed, and
the programme needed to turn a good fit into a meaningful DM constraint.
\end{abstract}
...
\paragraph{Fixed distance.} $D=5.43$~kpc throughout, except in one diagnostic
fit (Section~\ref{sec:diag}).
...
This property is \emph{consistent with the data}. Inside 1\pc the HST proper
motions give $\sigma_{{\rm pm},T}/\sigma_{{\rm pm},R}=1.000\pm0.007$ (10 bins;
$0.998\pm0.025$ inside 0.3\pc). The oMEGACat analysis also reports an
isotropic centre \citep{2025ApJ...983...95H}. The central value
$\beta_0=-0.5$ of the rung-2 Jeans fit is not a measurement. It is the upper
bound of that fit's prior, imposed because a cored tracer around a central
point mass requires $\beta_0\le-1/2$ for a positive DF
\citep{2006ApJ...642..752A}. The maximum-likelihood value, $-0.503$, sits on
that bound, so the data pull towards isotropy.
...
potential. Second, if future data showed central anisotropy.
...
\paragraph{No DM constraint yet.} The halo improves the objective by 31. This
cannot be read as a detection or as a preference. The halo solution has two
coordinates at bounds; the anisotropy rearranges to accommodate it; the halo
acts mainly beyond the outermost data; and the likelihood is not calibrated
(Section~\ref{sec:data-issues}).

\paragraph{The stellar/DM split is weakly identified.} Noiseless mock tests with
the one-transition family fitted a matched family
(\path{docs/COMPACT_DF_RECOVERY.md}). The fits reproduced the observables
to 0.006--0.02 errors RMS and recovered the total mass within 5\%. They did
not recover the split. In a no-DM mock the fits found
$\rho_{20}=1.4$--$2.3\,\Msun\pc^{-3}$ with 15--24\% less stellar mass; in a
cored mock ($\rho_{20}=2$) they found 2.5 and 3.1. The objective at the
injected truth ($2.3\times10^{-4}$) was lower than at the fitted solutions
($6\times10^{-3}$--0.1), but by amounts far below the $\chi^2$ scatter
expected with real noise, $\sqrt{2\times171}\simeq18.5$. Those mocks were
also not $\omega$ Cen-like: their dispersions were 35--40\% too low.

\paragraph{What a meaningful constraint requires.}
\begin{enumerate}
\item A model that passes objective~1, with no coordinate at a bound.
\item A \textbf{profile likelihood in $\rho_{20}$}: fix $\rho_{20}$ on a grid
  (e.g.\ 0, 0.25, 0.5, 1, 2, 4\,$\Msun\pc^{-3}$), refit all other coordinates
  including the anisotropy, and record the objective. $r_s$ is a nuisance
  parameter that the data do not constrain; the quoted quantity should be
  $\rho_{20}$ or $M_\dm(<r)$ within the data range.
\item \textbf{Validation on realistic mocks} built from the best no-halo and
  halo fits, with noise, to measure identifiability and coverage.
\item A \textbf{calibrated likelihood} before converting $\Delta\chi^2$ into
  intervals. Until the Gaia error model is settled, mock-calibrated
  thresholds are the substitute.
\end{enumerate}
...
\item The remaining misfits are localized at the MUSE outer edge and in the
  ...
  compilation.
\item Adding a halo trades against the anisotropy. Its preference is not
  interpretable yet; no DM constraint is claimed.
\end{enumerate}
...
(i) A $\rho_{20}$ profile likelihood with the two-transition DF.
(ii) Realistic mocks (v3) from the best fits, with noise, to validate
identifiability and coverage.
(iii) Intervals only after the likelihood is calibrated.
...
\texttt{dftwo\_halo\_20260923} & \texttt{bb40db1} & Two transitions + free cored halo, 6 starts\\
\bottomrule
\end{tabularx}
\end{table}
...
\path{plot_df_structure.py}, \path{plot_best_fit_performance.py} and
\path{plot_central_bh_evidence.py}. Plotted data and checksums are in
