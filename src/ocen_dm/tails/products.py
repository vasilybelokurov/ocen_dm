...
def build_products(out_dir: Path = Path("results/tails"), plot_dir: Path = Path("plots")):
    ...
    tab.meta.update(present=PRESENT,
                    note="class2: exponential stripping since infall; class3: GSE-debris orbit before "
                         "bar migration, static McMillan17, bar not modelled")
    ...
    titles = ["Class 1: present orbit, static potentials (backwards 10 Gyr)",
              ...
              "Class 3: GSE-debris orbit before bar migration (static McMillan17; bar not modelled)"]
    for ax, cls, title in zip(axes, ("class1", "class2", "class3"), titles):
        for o in S[cls]:
            ax.plot(-o.t_gyr if cls != "class3" else o.t_gyr, o.r, lw=0.8, label=o.label)
    ...
    axes[2].set_xlabel("time since start of the debris orbit [Gyr]")
