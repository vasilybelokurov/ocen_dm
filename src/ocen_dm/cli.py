"""Command-line interface for data preparation and spherical kinematic fits.

Use ``python -m ocen_dm.cli`` from a checkout with ``PYTHONPATH=src``, or the
installed ``ocen`` entry point. See ``--help`` for implemented commands and
``docs/MODELLING_PLAN.md`` for the adopted sequence. A bare ``fit`` retains the
legacy varying-anisotropy model with instrument scales; the isotropic baseline
requires ``--isotropic --no-scales`` and a fresh ``--label``.
"""
...
def cmd_fit(args: argparse.Namespace) -> int:
    ...
    kw["anisotropy_profile"] = getattr(args, "anisotropy_profile", "monotonic")
    if kw["anisotropy_profile"] == "turnover" and (
            args.preset or args.backend != "jeans" or args.isotropic or args.constant_beta):
        print("--anisotropy-profile turnover requires varying beta, --backend jeans and no preset", file=sys.stderr)
        return 2
    ...
    if args.mock_from:
        from .kinematics.report import load_run_metadata, problem_for
        from .kinematics.run_io import family_config, sha256
        source = Path(args.mock_from).expanduser()
        vector_path = source / "ml_x.npy" if source.is_dir() else source
        try:
            x_true = np.load(vector_path, allow_pickle=False)
            if (vector_path.parent / "summary.json").exists():
                generating_run = load_run_metadata(vector_path.parent)
                gen = problem_for(generating_run).family
                expected = ("K1" if not isinstance(gen, DarkMatterModel) else
                            "K2-cored" if gen.gamma == 0 else "K2-nfw" if gen.gamma == 1 else None)
                if args.mock_family is not None and args.mock_family != expected:
                    raise ValueError("--mock-family conflicts with the source run's model")
            else:
                if args.mock_family is None:
                    raise ValueError("bare --mock-from vectors need --mock-family; the tracer, "
                                     "backend and ladder flags then describe the generator too. "
                                     "Prefer a saved run directory or its ml_x.npy")
                gen = (NoDarkMatterModel(**kw) if args.mock_family == "K1" else
                       DarkMatterModel(gamma=0.0 if args.mock_family == "K2-cored" else 1.0, **kw))
            if x_true.shape != (len(gen.names),) or not np.all(np.isfinite(x_true)):
                raise ValueError(f"--mock-from needs a finite vector with {len(gen.names)} entries "
                                 f"for {gen.label}; got shape {x_true.shape}")
        ...
        data_provenance = {"kind": "mock", "generating_family": gen.label, "seed": args.seed,
                           "source_file": str(vector_path.resolve()), "source_sha256": sha256(vector_path),
                           "tracer": gen.tracer, "model": family_config(gen),
                           "noise_model": "gaussian_average_errors_clipped_v1",
    ...
    if out.exists():
        print(f"run directory already exists: {out}; choose a new --label", file=sys.stderr)
        return 2
    ...
    summary = run_nested(problem, out, n_live=args.n_live, dlogz=args.dlogz, seed=args.seed,
                         ...
                         dataset_options={"gaia_errors": args.gaia_errors,
                                          ...
                                          "gaia_r_min": args.gaia_r_min,
                                          "tracer": family.tracer, "backend": family.backend,
                                          "preset": preset.name if preset else None,
                                          "isotropic": family.constant_beta and family.fixed.get("beta_0") == 0.0,
                                          "constant_beta": family.constant_beta,
                                          "anisotropy_profile": family.anisotropy_profile,
                                          "no_scales": not family.nuisance_instruments,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    from .dynamics.cli import register as register_dynamics
    register_dynamics(subparsers)
    ...
    fit.add_argument("--anisotropy-profile", choices=["monotonic", "turnover"], default="monotonic",
                     help="radially varying beta: one transition (legacy) or two ordered transitions (rung 2)")
    ...
    fit.add_argument("--mock-from", default=None,
                     help="source run directory or .npy vector; source metadata defines the generating model")
    fit.add_argument("--mock-family", default=None, choices=["K1", "K2-cored", "K2-nfw"],
                     help="required for a bare vector without run metadata; ladder/tracer/backend flags "
                          "then also define the generator")
